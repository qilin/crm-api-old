...
func (a *UserMapRepo) FindByExternalID(ctx context.Context, storeId int, externalID string) (*domain.UserMapItem, error) {
	db := trx.Inject(ctx, a.db)
	var (
		out = &domain.UserMapItem{}
		e   error
	)
	e = db.Where("store_id = ? AND external_id=?", storeId, externalID).First(out).Error
	return out, e
}
