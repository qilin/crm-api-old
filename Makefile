...
define build_resources
	find "$(ROOT_DIR)/assets" -maxdepth 1 -mindepth 1 -exec cp -R -f {} $(ROOT_DIR)/.artifacts/${1} \; && \
	find "$(ROOT_DIR)/api" -maxdepth 1 -mindepth 1 -exec cp -R -f {} $(ROOT_DIR)/.artifacts/api/${1} \; && \
	find "$(ROOT_DIR)/configs" -maxdepth 1 -mindepth 1 -exec cp -R -f {} $(ROOT_DIR)/.artifacts/configs/${1} \;
endef
...
generate: init docker-protoc-generate vendor gqlgen-generate vendor go-generate ## execute all generators
.PHONY: generate
...
init:
	...
	mkdir -p generated $(ROOT_DIR)/.artifacts/configs $(ROOT_DIR)/.artifacts/api
.PHONY: init
